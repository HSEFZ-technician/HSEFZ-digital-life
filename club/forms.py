...
class RegisterForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            Div(
                HTML(
                    '{% load static %}<img class="form-icon" src="{% static \'256x256.jpg\' %}" alt="校徽">'),
            ),
            ...
            Field('realname', css_class="form-control item",
                  placeholder='名字'),
            AppendedText('email', settings.EMAIL_SUFFIX,
                         css_class="form-control item", placeholder='学校邮箱'),
    ...
    def clean_username(self):
        ...
        if re.fullmatch(student_id_validator, value) == None:
            raise forms.ValidationError('学号必须是%d位数字!' % (
                settings.STUDENT_ID_LENGTH), code='invalid')

        return value

    def clean_realname(self):

        realname_validator = r'^[\u4e00-\u9fa5]{' + str(
            settings.REALNAME_MIN_LENGTH) + r',' + str(settings.REALNAME_MAX_LENGTH) + r'}$'

        value = self.cleaned_data['realname']

        if re.fullmatch(realname_validator, value) == None:
            raise forms.ValidationError('姓名必须是%d~%d个汉字!' % (
                settings.REALNAME_MIN_LENGTH, settings.REALNAME_MAX_LENGTH), code='invalid')
        else:
            return value
    ...
    def clean_email(self):
        ...
        if re.fullmatch(email_validator, value) == None:
            raise forms.ValidationError(
                '邮箱只能包含大小写字母、.、下划线、数字!', code='invalid')
        ...
        user = StudentClubData.objects.filter(
            username=value, student_real_name=student_realname)
...
class ManualActivateForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            HTML('{% for message in messages %}\n<div {% if message.tags %} class ="{{ message.tags }}" {% endif %}><span class="password_or_username_invalid"><strong>{{ message }}</strong></span></div>\n{% endfor %}'),
            ButtonHolder(
                Submit('submit', '发送激活邮件',
                       css_class="btn btn-block submit_button")
            ),
        )
...
class SettingModifyPasswordForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            HTML(
                '<button class="btn btn-primary submit_button" id="submit-id-submit">提交</button>'),
            # ButtonHolder(
